...
# mydb=mysql.connector.connect(host='localhost',username='root',password='Abhi@mysql1289',db='ecommy')
user=os.environ.get('RDS_USERNAME')
db=os.environ.get('RDS_DB_NAME')
password=os.environ.get('RDS_PASSWORD')
host=os.environ.get('RDS_HOSTNAME')
port=os.environ.get('RDS_PORT')
with mysql.connector.connect(host=host,password=password,db=db,user=user,port=port) as conn:cursor=conn.cursor()
cursor.execute("CREATE TABLE if not exist admincreate(username varchar(50) NOT NULL,email  varchar(100) NOT NULL,password varbinary(10) NOT NULL,address text NOT NULL,accept enum('on','off') DEFAULT NULL,dp_image varchar(50) DEFAULT NULL,ph_no bigint DEFAULT NULL,PRIMARY KEY (email),UNIQUE KEY ph_no (ph_no)")
cursor.execute(" CREATE TABLE if not exist contact (name varchar(50) DEFAULT NULL,email varchar(50) NOT NULL,message text,Sno int NOT NULL AUTO_INCREMENT,UNIQUE KEY Sno (Sno))")
cursor.execute("CREATE TABLE if not exist items (item_id binary(16) NOT NULL,item_name varchar(255) NOT NULL,quantity int unsigned DEFAULT NULL,price decimal(14,4) NOT NULL,category enum('home','grocery','Fashion','Electronic') DEFAULT NULL,image_name varchar(255) NOT NULL,added_by varchar(50) DEFAULT NULL,description longtext,PRIMARY KEY (item_id),KEY added_by (added_by),CONSTRAINT items_ibfk_1 FOREIGN KEY (added_by) REFERENCES admincreate (email) ON DELETE CASCADE ON UPDATE CASCADE)")
cursor.execute("CREATE TABLE if not exist items (item_id binary(16) NOT NULL,item_name varchar(255) NOT NULL,quantity int unsigned DEFAULT NULL,price decimal(14,4) NOT NULL,category enum('home','grocery','Fashion','Electronic') DEFAULT NULL,image_name varchar(255) NOT NULL,added_by varchar(50) DEFAULT NULL,description longtext,PRIMARY KEY (item_id),KEY added_by (added_by),CONSTRAINT items_ibfk_1 FOREIGN KEY (added_by) REFERENCES admincreate (email) ON DELETE CASCADE ON UPDATE CASCADE)")
cursor.execute("CREATE TABLE if not exist reviews (username varchar(30) NOT NULL,itemid binary(16) NOT NULL,title tinytext,review text,rating int DEFAULT NULL,date datetime DEFAULT CURRENT_TIMESTAMP,PRIMARY KEY (itemid,username),CONSTRAINT reviews_ibfk_1 FOREIGN KEY (itemid) REFERENCES items (item_id) ON DELETE CASCADE ON UPDATE CASCADE)")
cursor.execute("CREATE TABLE if not exist usercreate (name varchar(50) NOT NULL,user_email varchar(50) NOT NULL,password varbinary(20) DEFAULT NULLaddress text,gender enum('male','female') DEFAULT NULL,PRIMARY KEY (user_email))")
...
def forgotpassword():
    if request.method=='POST':
        ...
        if count==0:
            ...
            return redirect(url_for('new',data=data))
        else:
            return 'something went wrong'
    ...
    try:
        ...
        if request.method=='POST':
            ...
            if npassword==cpassword:
                ...
                return 'your password is not matche'
    return render_template('newpassword.html')
@app.route('/dashboard/<category>')
def dashboard(category):
    cursor=mydb.cursor(buffered=True)
    cursor.execute('select bin_to_uuid(item_id),item_name,description,price,quantity,image_name from items where category=%s',[category])
    cursor.close()
...
def invoice(ordid):
    if session.get('useremail'):
        ...
        qty=orders[3]
        cost=orders[4]
        cursor.execute('select name,address,user_email from usercreate where user_email=%s',[username])
        data=cursor.fetchone()
        uname=data[0]
        uaddress=data[1]
        html=render_template('invoice.html',uname=uname,uaddress=uaddress,oname=oname,qyt=qty,cost=cost)
